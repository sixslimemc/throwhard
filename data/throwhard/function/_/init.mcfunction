...
data modify storage throwhard:config throw.item set value {enable:true, fast_pickup:true, add:false, force:{transfer_player_motion:true, base:0.27d, fling:{multiplier:0.8d}, variation:{rotation:{radial:{min:0, max:5}}}}}
...
data modify storage throwhard:config throw.splash_potion set value {enable:true, add:false, force:{transfer_player_motion:true, base:0.35d, fling:{multiplier:0.6d}, variation:{magnitude:{min:-0.03, max:0.07}, rotation:{radial:{min:0, max:4}}}}}
